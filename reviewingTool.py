...
reviewingTool_bp = Blueprint('review', __name__, template_folder='templates')
...
id2label = {}
# replace your Huggingface access token here
#auth_token = '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'

# define model name or path
#model_path = 'atatavana/passive_invoices_v4.7.1'
...
@reviewingTool_bp.route('/review', methods=['POST'])
def reviewPage():
    '''
    data = {
        'image1.jpg': {
            'LMCU117245/7': [(990, 455, 1310, 495), ['container id', 'others'], False],
            'M2191390': [(1180, 530, 1390, 580), ['seal number', 'tare'], False],
        },
    }
    '''
    ...
    return render_template('review.html', data=data, label2color=json.dumps(label2color), model_path=model_path, auth_token=auth_token)
...
@reviewingTool_bp.route('/review/perform-ocr/<image_name>')
...
@reviewingTool_bp.route('/review/predict-labels/<image_name>')
def predictLabels(image_name):
    ...
    try:
      ...
      if True:
          ...
          comparison_df.to_csv(new_csv_path, index=False)
...
def compare_csv(annotation_path, prediction_path, isDataFromReviewed = False):
    annotation_df = pd.read_csv(annotation_path).fillna(' ')
    prediction_df = pd.read_csv(prediction_path).fillna(' ')
    ...
    for index, ann_row in annotation_df.iterrows():
        ...
        pred_row = prediction_df.query('word == @word and xmin == @xmin and xmax == @xmax and ymin == @ymin and ymax == @ymax')
        labelAreSame = not pred_row.empty and pred_row.iloc[0]['label'] == label
        
        image_data.append({
            'index': index,
            'word': word,
            'coords': [xmin, ymin, xmax, ymax],
            #'labels': [label, pred_row.iloc[0]['label'] if not pred_row.empty else ''],
            'labels': [label, pred_row.iloc[0]['label']] if not labelAreSame and not pred_row.empty else [label],
            'labelAreSame': labelAreSame,
            'correctLabel': label
            #'correctLabel': pred_row.iloc[0]['label'] if isDataFromReviewed and not pred_row.empty else label
        })

    return image_data

@reviewingTool_bp.route('/review/get-image-data/<image_name>')
def get_image_data(image_name):
    ...
    if os.path.exists(reviewed_csv):
            data[image_name] = compare_csv(reviewed_csv, annotation_csv, isDataFromReviewed=True)
    elif os.path.exists(prediction_csv):
            data[image_name] = compare_csv(annotation_csv, prediction_csv)
    elif os.path.exists(annotation_csv):
            data[image_name] = compare_csv(annotation_csv, annotation_csv)
    return data[image_name]


def populateLabel2Color(id2label):
    np.random.seed(42) 
    colors = ['blue', 'cyan', 'dodgerblue', 'violet','darkviolet', 'lime', 'brown', 'lightcoral', 'darkorange', 'gold', 'olivedrab']
    label2color = {}
...
@reviewingTool_bp.route('/review/save-image-data', methods=['POST'])
def save_image_data():
    data = request.json
